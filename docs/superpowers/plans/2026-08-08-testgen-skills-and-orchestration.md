...
- **When a test derives both sides from one source, mutate the code path between them — never the source.** This plan has now made the same mistake twice, in the same way, and the second time it was in an instruction telling an implementer how to verify a fix. Task 4: a `discriminating_fact` test read `_SCENARIOS`, and so did the builder it was checking, so mutating `_SCENARIOS` moved both sides and proved nothing — the real mutation site was the copy inside `toy_expected()`. Task 5: an assertion compared an emitted package against `toy_expected(sid)`, and I proposed verifying it by swapping `_ORACLES`, which feeds both. Also inert. **The test may still be perfectly load-bearing** — both were — but the mutation that demonstrates it has to sit on the path *between* the two sides: the copy, the compile step, the writer. Before writing "verify by mutating X", trace whether X reaches both sides of the assertion. If it does, name a different site.
...
**The four tests marked "Strengthened … under a Task 11 ruling" below replaced weaker
versions during Task 11; do not "restore" the whole-body substring forms.** The originals
were `"closed" in body.lower()`, `"refusal" in body.lower() or "answers nothing" in
body.lower()`, `"distractor" in body and "hop_depth" in body`, and an ordering check over
`body.find("1.")`…`body.find("6.")`. Task 11's implementer flagged them as substring floors;
the reviewer then computed each against the delivered skill and found three satisfied by
content unrelated to the property they name, and the fourth constraining only half of the
ordering it claimed:

- `"closed"` matched "closed objects"/"a closed object" — both about JSON
  `additionalProperties`, neither about the assertion vocabulary. Deleting the sentence that
  says the vocabulary cannot be extended left the test green. Same shape as Task 10's
  hole-reason test being satisfied by a `rejected_reason` enum elsewhere in the file.
- `"refusal"` matched the mandatory `## 5. Refusal conditions` heading itself: `SECTIONS`
  requires that heading and `load` sets `body` to the whole file, so the clause held for
  every conforming skill in this build, permanently and regardless of content.
- `"distractor"`/`"hop_depth"` matched the frontmatter description plus section 1's list of
  the scenario fields this stage reads. Method step 2 could be deleted outright and it passed.
- The ordering markers `"1."`, `"2."` and `"3."` resolved to the *section headings*
  (`## 1. Inputs`, `## 2. Output`, `## 3. Method`), not to Method steps — so only 4 → 5 → 6
  was constrained, and the distractors-before-seed order the docstring names first was not.

The fix scopes each to `section_body(skill, "3. Method")` — public and fence-aware, and its
docstring already anticipates this use — and, where the property is really a co-occurrence
(a rule and the remedy it prescribes), asserts it within a single paragraph. Evidence
recorded in Task 11's report: under each of four deletion mutations, all four *original*
predicates still return `True` while exactly one strengthened test fails, and it is the
named one. **This is the plan's one departure from "the test file is verbatim", and it was
ruled by the human partner rather than assumed** — the general lesson is in the Tasks 7–13
preamble: a prompt test must assert a structural property, and "a substring of the whole
`body`" is not one when `body` includes the frontmatter and the mandated headings.
...
- Build a toy run stopped after `score` (`upto="score"`), then **delete `04-instances/` entirely** so the stage has real work. **Verified during Task 11: at this checkpoint there is nothing to delete** — `build_toy_run` returns before its instantiate block and `paths.py` contains no `mkdir` at all, so `04-instances/` does not exist. Keep the step as a confirmation, because a run built past `score` would hand the fan-out the fixture's own hand-authored seeds and every property below would read as a pass.
...
**The three tests whose docstrings below record a strengthening, and the
`reads` pin in `test_the_contract_matches_the_stage_gate`, replaced weaker versions during
Task 12; do not "restore" the whole-body substring forms.** The originals were
`body.count("uniquely_determined") >= 2`, `"last" in method or "only then" in method`, and
`"disagree" in body.lower()`; the contract test asserted only `stage` and `schemas`. Task 12's
implementer flagged the first three as substring floors rather than patching them silently, and
the reviewer then computed each against the delivered skill:

- `count("uniquely_determined") >= 2` — the field is named five times in the delivered
  `SKILL.md` (section 2's field list, section 2's conditional-fields bullet, Method step 2 where
  the judgment is made, and twice in section 4). Deleting Invariant 3, the one sentence that
  states the incompatibility with `accept`, left four occurrences and the test green. Now a
  co-occurrence of `accept` + both negatives inside one `## 4. Invariants` paragraph.
- `"last" or "only then"` over the whole Method section — `"last"` occurred nowhere in Method,
  so the assertion rested entirely on `"only then"` in step 4's heading, and the `"last"` disjunct
  is satisfiable by content unrelated to the ordering: this fixture's own `scn-blocked`
  discriminating fact is *"only its last comment names the blocker"*, so a future edit quoting it
  inside Method would keep the test green with the ordering sentence deleted — on the **only**
  automated guard this skill has on its central property. Now a co-occurrence in the Method
  paragraph that *first* names `expected.json`, which is the paragraph a model reading in order
  acts on.
- `"disagree" in body.lower()` — `"disagree"` occurs six times in the delivered skill and two
  are unrelated to the property ("a denormalised count that disagrees with the records it counts";
  a `scenario_id` that "disagrees with the name it is filed under"). So the section-5 condition
  *and the brief-supplied verdict-table row 4 itself* could both be deleted with the test green,
  on the row this plan calls the pipeline's highest-value catch. Now a co-occurrence of
  `expected.json` + "oracle looks wrong" + "disagree" in one `## 5. Refusal conditions` paragraph.
- The `reads` pin is new rather than strengthened, and it is load-bearing. `check_contract`
  requires each `reads` entry to be a public `RunPaths` attribute but never checks *which*
  entries, so `world_model` could be added to this adversary's context and every test in the file
  would still pass — verified. The human partner ruled `world_model` **deliberately withheld**
  (`refs._check_invariants` already evaluates every `machine:` invariant over every seed and the
  orchestrator runs `check-refs`, so declaring it buys no coverage and adds anchoring surface to
  the one stage whose entire value is not being anchored), which is the opposite of Task 10's
  `manifest` ruling because nothing else supplied `max_rounds` there. The pin is what keeps a
  later well-meaning edit from quietly widening the slice. **The general test, binding on Task
  13: before declaring a withheld artifact, ask whether a deterministic gate already enforces the
  property — if it does, the requirement belongs to the gate and the prose should say so.**

Evidence recorded in Task 12's fix report: under four deletion mutations — Invariant 3 deleted;
the preamble's ordering word dropped; the ordering deleted from *both* the preamble and step 4's
heading with `scn-blocked`'s fact quoted into Method; and section 5's row-4 condition deleted
together with verdict-table row 4 — every *original* predicate still returns `True` while exactly
one strengthened test fails, and it is the named one. Adding `world_model` to `reads` leaves
`check_contract` clean and the pre-fix contract-test body passing, and fails the pinned test.

**One refusal condition was also retitled in fix round 1.** The table row above reads "The seed
violates a world-model invariant", which over-promises relative to what a stage without
`world_model` in `reads` can detect. The delivered skill states the trigger as **"The seed
contradicts itself"** — a count disagreeing with the records it counts, an id used twice, a
dangling reference — and says explicitly that enumerating the invariants is not this stage's job
because `refs._check_invariants` already does it. Read the row above as shorthand for that.
...
**Two of the tests below replaced weaker versions during Task 13's fix rounds, and a third grew a `reads` pin; do not "restore" the whole-body substring forms, and note that this block is the *delivered* file, kept byte-for-byte in step with `tests/unit/test_skills_emit.py`.** The two originals were `"reproducib" in body` and `"prune" in body and "rejected" in body`. Both were measured satisfiable by content unrelated to the property they name: `"prune"` by the frontmatter `description:` line and `"rejected"` by an incidental mention of `check-refs`'s tolerance in Method step 3, and `"reproducib"` by Invariant 1's word *unreproducible* — a consequence of hand-editing a package, not a reason emit is code. Deleting every statement of either property left all five tests green. Each docstring below records its own measurement.
...
**Seven of the tests below replaced weaker versions during Task 13's fix rounds, and one was added (the `reads` pin); do not "restore" the whole-body substring forms, and note that this block is the *delivered* file, kept byte-for-byte in step with `tests/unit/test_skills_orchestrate.py`.** Two more of the same class live in Task 13a's block, for nine in the task altogether. Each strengthened docstring records the measurement that condemned its predecessor. The pattern across all of them: `load()` sets `body` to the **entire file**, so a whole-body substring is satisfiable by the frontmatter `description:` line (which alone satisfied every predicate of the three-things test) or by the mandatory `## Contract` block (whose `invokes` list alone supplied both `record-stage` and `decide`, and the `smoke` half of the every-stage test). All nine stayed green under a deletion of the very prose their docstring names; what differed was the carrier -- the frontmatter line for two, the mandatory contract block for two, and ordinary prose elsewhere in the file for the rest. Two rounds were needed because a strengthening can also fail by **over-coupling**: the first replacement for the exit-code test anchored on the bold `| **0** |` table cell and broke on a prose rewrite that preserved the rule, and the first replacement for the human-gate test pinned three `Human gate N` labels while a file reducing all three gate steps to bare stubs still passed.
