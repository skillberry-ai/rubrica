...
- `check_scenario_parts(run)` — for each round in `run.scenario_part_rounds()`, read `run.batches(round_n)` for **that round's** batch roster (Ruling R9 — never a single shared file, or round 1's parts get checked against round 2's assignment) and report a batch with no part file, a part whose `batch_id` disagrees with its filename, a part name that is not a safe segment, and — the check that makes the partition enforceable — any scenario one of whose **`provenance.hole_refs`** entries is not in its own batch's `hole_refs`. Report the offending scenario id, the ref, and the batch that owns it, so the message names all three.

**Resolve `provenance.hole_refs`, never `goal_id` or `capability_refs` (Ruling R21).** `scenarios-0.1.json` already requires `provenance.hole_refs` with `minItems: 1`, so a scenario has always declared exactly which holes it was written to close — no schema change is needed, and that field is the only one that carries the answer.

An earlier version of this plan checked `goal_id` and `capability_refs` instead, which **could not be satisfied at all.** MEASURED against a real `write_batches` partition (4×5 cells + 2 goals → `b01` = 17 cells, `b02` = 3 cells + 2 goals): **19 of 22 correct scenarios were reported**, and the minimal fixture was degenerate — neither batch could be given a legal scenario. The error was conflating *the ids a scenario names* with *the holes it targets*. A scenario closing `cell:X/Y` still has to name a `goal_id`, required and drawn from the world model's frozen list, and that goal is not a hole assignment; `partition` chunks the sorted ref list so `cell:` clusters before `goal:`, so a cell-only batch's member necessarily names a goal another batch owns.

`provenance.hole_refs` is also the faithful analogue of the triage precedent this check was modelled on: a `dispositions-part` names a `candidate_id` and the seal refuses one outside its own slice, because the candidate *is* the work unit. Here the work unit is a hole, and `provenance.hole_refs` is where the member names it. It keeps the check at layer 2 as well — it verifies a reference is in scope, never that the scenario actually *closes* the hole, which is semantic. Anchor findings on the part path, never on `run.scenarios`.
