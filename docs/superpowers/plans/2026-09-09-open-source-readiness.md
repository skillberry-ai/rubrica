...
| `CHANGELOG.md` | The `# Changelog` header and nothing else; `release.sh` prepends every section after it. **Corrected 2026-09-09:** the compare links this row and the design's practice 4 called for are *not* delivered — `release.sh` mints none, and a seeded `[Unreleased]:` definition both dangles and sinks below the sections. See the design's practice 4 and `docs/releasing.md`. |
