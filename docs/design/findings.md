...
id — true *within* a round and false *across* rounds. Observed on
`run-20260906-102327` (tau2-retail), round 2: its batch was planned as `b01`, the
dispatched member complied and minted `sc-b01-01` .. `sc-b01-05`, colliding with
round 1's five. `propose-seal` exited 1 and wrote nothing, layer 2 named the
...
cause, visible only in the transcript: both of round 2's dispatches read
...
match. Two guards were added, each watched failing first:
