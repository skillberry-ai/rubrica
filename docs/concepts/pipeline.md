...
| — | `triage-rule` | `rb-triage-rule` — fan-out, one per slice | its own `00-slices/<slice_id>.json` shard, and `00-objective.json` — never a sibling's shard | `00-dispositions/<slice_id>.json` | validate · check-refs |
