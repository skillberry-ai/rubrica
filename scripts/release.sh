...
#
# This script does not publish the package. Publishing the *release* does:
# .github/workflows/publish.yml triggers on `release: published` and uploads to
# PyPI with a trusted publisher — see docs/releasing.md, "PyPI".
#
# Usage:
...
Environment overrides (used by tests/unit/test_release.py):
  RELEASE_REMOTE    git remote to push to (default: origin)
  RELEASE_GH_REPO   HOST/OWNER/REPO for gh (default: github.com/skillberry-ai/rubrica)
...
: "${RELEASE_GH_REPO:=github.com/skillberry-ai/rubrica}"
: "${RELEASE_SKIP_GH:=0}"
...
# separated. When the new section is the last thing in the file — which is the case
# for the first release against a header-only CHANGELOG — that leaves a trailing
# blank line. Normalize the assembled file to exactly one trailing newline:
# command substitution strips every trailing newline, then add one back. Internal
# blank lines, including the separators between sections, are untouched. The
# invariant is pinned by tests/unit/test_release.py, and it is what keeps an
# end-of-file-fixer style commit hook from rewriting a staged file and failing the
# release commit.
...
git commit -S -s -m "chore(release): $TAG" \
    || die "the release commit failed — see the output above. Most likely signing,
which is mandatory: do not retry without it. A commit hook that rewrote a staged
file would fail the commit the same way. The worktree is restored below."
...
echo
info "Released $TAG."
info "Publishing the release fires publish.yml — inert until a PyPI trusted publisher exists."
