...
def test_closable_holes_does_not_offer_an_undrivable_cell_on_round_two(tmp_path):
    ...
    `not_yet_attempted` the one wrong reason there, which is why this test asserts
    against the *filter* and not against the prompt: a prompt change is validated by
    a dispatch, none has been run, and a score part written before the change is
    still a part this code has to read.
