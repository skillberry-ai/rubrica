...
def _reconcile_parts(run: RunPaths) -> None:
    """Every partial the seal reads, and no world model yet.

    Seven files, which is not one per prompt pass in the family: `01-services.json`
    is deliberately absent, because the seal does not read it and a state that
    always wrote it could not show that. `01-interfaces/` is absent for the same
    reason one step on -- it is derived from the part this state omits.
