...
def test_no_schema_pattern_uses_a_caret_dollar_anchor(kind, tmp_path):
    ...
    def resolve(ref: str, home: str) -> tuple[dict, str]:
        ...
        Refs are followed rather than stopped at, because the triage part
        schemas own no `pattern` of their own: their ids are
        `triage-0.1.json#/$defs/id`, resolved at validation time through
        validate._schema_registry. Stopping at the ref would make `found` empty
        for all five of them, and the `assert found` guard below -- which is
        what stops this test passing vacuously -- would have to be dropped. This
        way it keeps its teeth, and gains some: a part that pointed at a
