...
# -- unreadable inputs on the world-model path --------------------------
def test_an_unreadable_claims_directory_is_a_usage_error_not_a_finding_list(tmp_path):
    ...
    Measured: check_world_model raises, and the raise is the designed answer.
    `_claim_ids` reads the directory through `paths.list_json`, which raises
    deliberately (paths.py:131-137) because an unreadable run directory is "the
    harness pointed at something it cannot read, which is exit 2" -- not a
    repairable stage defect at exit 1.

    Asserted in that direction on purpose. The tempting shape of this test -- one
    that expects a finding list back beside an unreadable claims directory -- could
    only pass if `_claim_ids` went back to swallowing EACCES, which is the exact
    regression paths.py exists to prevent. So what this pins is that no partial
    finding list escapes and nothing invents a claim defect.
    """
    run = _run(tmp_path)
...
def test_a_world_model_read_through_a_mode_0444_file_is_checked_normally(tmp_path):
    ...
    gate 1 leaves behind.

    Asserted through a world model that *has* a defect, not a clean one: a clean
    fixture returns `[]` whether the file was read or silently skipped, so the
    chmod would be decoration. A checker that could not read the document reports
    ...
    assert any("capability_cells" in f.message for f in findings)


def test_check_all_over_a_write_protected_run_reports_instead_of_raising(tmp_path):
    """A readable-but-unwritable run is still a run every checker can answer for.
    The exit-code contract's second invariant is that a `1` never has empty stdout,
    and an exception escaping the handler produces exactly that -- so a checker
    that raises here would turn a stale denominator into an unrepairable `2`."""
    run = _run(tmp_path)
    world = minimal_world_model()
    world["denominator"]["capability_cells"] = 5
    write_json(run.world_model, world)
