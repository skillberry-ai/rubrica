...
# reconcile-seal and triage-seal are here for a different reason from the other
# three: nothing about assembling a family's staged parts is judgment, and two
# runs with identical parts must produce a byte-identical output or variance
# stops being attributable to the pass that caused it -- the same ruling that
# makes emit code. See src/rubrica/reconcile.py and src/rubrica/seal.py.
